...
exports.deploy = function (target, options) {
  promptForTarget(target, options.directory, function (targetPath) {
  ...
  inquirer.prompt([
    {
      type: 'list',
      name: 'target',
      message: 'Whic target would you like to deploy?',
      choices: files
    }
  ], function (answers) {
    cb(answers.target);
...
function promptForTarget(target, directory, cb) {
  if (!target) {
    ...
    if (directories && directories.length === 1) {
      cb(path.join(directory, directories[0]));
    }
    else {
      targetPrompt(directories, function (target) {
        cb(path.join(directory, target));
      });
    }
  }
  else {
    cb(normalizeTarget(target, directory));
  }
}
