...
var projectPackage = path.join(process.cwd(), 'package');
var pkg;

if (!fs.existsSync(projectPackage)) {
  console.error(chalk.red('Could not find `package.json` in this directory. Paratrooper should be run from the project root, where `package.json` is located.'));
}
else {
