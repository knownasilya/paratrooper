...
function createRemoveScript(settings) {
  var apppath = path.join(settings.serverAppPath, settings.appName);
  var nginxConfig = path.join(settings.sitesEnabledPath, settings.appName);
  var upstartConfig = path.join('/etc/init', settings.appName + '.conf');
...
function remove(settings) {
  execute(settings.sshAddress, createRemoveScript(settings));
}
